#This function reads the data of outcome of DMFT+CTAUX and defines the staggered magnetization
# according to as mentioned in the draft.
# m_s = m_{\lambda_x}- m_{-\lambda_x}
# where m_{\lambda_x}= n_{\uparrow \lambda_x}-n_{\downarrow \lambda_x}
...
#plt.plot(U,abs(Nup[:,1]+Ndw[:,1]-Nup[:,0]-Ndw[:,0]),'go-',lw=2.5)
plt.legend([U05, U1, U4, U5, U6], ['U=0.5','U=1.0','U=4.0','U=5.0','U=6.0'],loc='upper right')
plt.ylim(-0.1,1.6)
plt.xlim(0.25,3.00)
plt.ylabel(r'$m_s$', fontsize=40)
plt.xlabel(r'$\lambda_x/t$', fontsize=40)
plt.grid(True)
plt.show()

#plt.savefig('stag_mag.png')
